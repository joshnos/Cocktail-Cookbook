...
import '../../../application/categories_list_cubit/categories_list_cubit.dart';
import '../../utils/widgets/drink_card.widget.dart';

class DrinksByCategory extends StatelessWidget {
  const DrinksByCategory({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocListener<CategoriesListCubit, CategoriesListState>(
            ...
            child: SingleChildScrollView(
              child: Wrap(
                ...
                children: state.filterList!.map(
                  (e) {
                    return FilterChip(
                      label: Text(e.strCategory ?? 'Category'),
                      selected: state.selected == e.strCategory,
                      onSelected: (value) {
                        if (value) {
                          context
                              .read<CategoriesListCubit>()
                              .getDrinksByCategory(e.strCategory!);
                        }
                      },
                    );
                  },
